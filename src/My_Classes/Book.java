...
public class Book {
    ...
    public Book(String _isbn, String _name, String _author, String _genre, Integer _quantity,
...
//    }
    ...
    public void addBook(String _isbn, String _name, String _author, String _genre, Integer _quantity,
                        String _publisher, double _price, String _date_received, String _description, byte[] _cover) {
        String insertQuery = "INSERT INTO `books`(`isbn`, `name`, `author`, `genre`, `quantity`, `publisher`,"
    ...
    public void editBook(String _isbn, String _name, String _author, String _genre, Integer _quantity,
                         String _publisher, double _price, String _date_received, String _description, byte[] _cover) {
        String updateQuery = "";
        ...
        try {
            ...
            {
                updateQuery = "UPDATE `books` SET `name`=?,`author`=?,`genre`=?,`quantity`=?,`publisher`=?,`price`=?,`date_received`=?,`"
                        + "description`=?,`cover_image`=? WHERE `isbn`=?";
                ps = DB.getConnection().prepareStatement(updateQuery);

                //ps.setString(1, _isbn);
                ps.setString(1, _name);
                ps.setString(2, _author);
                ...
                ps.setString(10, _isbn);
                //ps.setInt(10, _id);
            }
//            } else {
//                updateQuery = "UPDATE `books` SET `name`=?,`author`=?,`genre`=?,`quantity`=?,`publisher`=?,`price`=?,`date_received`=?,`description`=? WHERE `isbn`=?";
//                ps = DB.getConnection().prepareStatement(updateQuery);
//
//                //ps.setString(1, _isbn);
//                ps.setString(1, _name);
...
//            }
    ...
    public Book searchBookById_Isbn(String _isbn) {
        String query = "SELECT * FROM `books` WHERE `isbn` = '" + _isbn + "'";
        ...
        try {
            if (rs.next()) {
                ...
                //isbn, name, author, genre,quantity, publisher,price, date, description, cover
            } else {
                return book;
            }
        } catch (SQLException ex) {
